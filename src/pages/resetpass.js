...
const Reset = () => {
  ...
  const resetpassfunc = (e) => {
    ...
    setNewpass1({ ...newpass, [name]: value });
  };

  const storedata4 = async (e) => {
    e.preventDefault();
    const { email, password } = newpass1;
  ...
  return (
    <div className="container-fluid" style={{ backgroundColor: '#e91e63', width: "100%", height: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center', margin: 0, padding: 0 }}>
      <form style={{ backgroundColor: 'white', padding: '20px', borderRadius: '8px' }} onSubmit={storedata3}>
        <div className="mb-3">
          <label htmlFor="exampleInputEmail1" className="form-label">Email address</label>
          <input type="email" className="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" name="email" value={newpass.email} onChange={newpassfunc} />
        ...
        <div className="mb-3">
          ...
          <input type="password" className="form-control" id="exampleInputPassword1" name="password" value={newpass.password} onChange={newpassfunc} />
        </div>
        <div className="mb-3">
